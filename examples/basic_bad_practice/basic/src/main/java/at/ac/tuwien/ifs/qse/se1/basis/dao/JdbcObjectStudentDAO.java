 ...
 * ({@value at.ac.tuwien.ifs.qse.se1.basis.helper.Constants#SPRINGBEANS})
 * for configuration.
 * 
 * @author The SE-Team
 * @version 1.0
 * @see StudentDAO
 * @see <a href="http://en.wikipedia.org/wiki/Object_factory_pattern"
 *      target="newWindow">wikipedia on Factory Pattern</a>
 * @see <a href="http://en.wikipedia.org/wiki/Data_Access_Object"
 *      target="newWindow">wikipedia on DAO</a>
 */
public class JdbcObjectStudentDAO implements IStudentDAO {
	...
	private class BaseStudentsQuery extends MappingSqlQuery {
		...
		protected Student mapRow(ResultSet rs, int rowNumber)
			...
			student.setId((Long) rs.getObject(1));
			student.setMatnr(rs.getString(2));
	...
	/**
	 * Retrieves all students in the DB. This is a private inner class,
	 * as this is used only in this data access object.
	 */
	private class Query_GetAllStudents extends BaseStudentsQuery {
		/**
		 ...
		 * @see #dataSource
		 */
		public Query_GetAllStudents(DataSource ds) {
			super(ds, sql_getAllStudents);
			compile();
		}
	}
	...
	private class Query_GetStudent extends BaseStudentsQuery {
		...
		public Query_GetStudent(DataSource ds) {
			super(ds, "select id, matnr, vorname, nachname, email from studenten where id=?");
	...
	private class Query_GetStudentID extends MappingSqlQuery {
		...
		protected Object mapRow(ResultSet rs, int rowNumber)
			...
			// the ID
			Long id = new Long(rs.getLong("@p0"));
			return id;
		}
	}
	...
	public Student getStudent(long id) {
		...
		List students = new Query_GetStudent(dataSource).execute(id);
		if (students.size() == 1) {
	...
	public Student saveStudent(Student student) {
		...
		new Query_InsertStudent(dataSource).update(param);

		// get the id of the just inserted student
		List results = new Query_GetStudentID(dataSource).execute();
	...
	public Student updateStudent(Student student, long id) {
		log.debug("Update Student, ID = " + student.getId() + " new ID = "
				+ id);
		log.debug("Initialise SQL Parameters");
		final Object[] param = new Object[] { student.getMatnr(),
		...
		if (new Query_UpdateStudent(dataSource).update(param) == 1) {
	...
	/**
	 * 
	 * A comparator helper class to sort objects of type Student according
	 * to Matrikelnummer and Nachname. 
	 *
	 */
	private class StudentComparator implements java.util.Comparator<Student> {
		
		private String order;
		public StudentComparator(String order) {
			this.order = order;
			if (order != "Matrikelnummer" && order != "Nachname") {
				log.warn("Undefined sort order for students, defaulting to Nachname");
				this.order = "Nachname";
			}
		}

		// the compare method according to the comparator interface
		public int compare(Student s1, Student s2) {
			if (order.equals("Matrikelnummer"))
				return s1.getMatnr().compareTo(s2.getMatnr());
			else
	...
	public List<Student> getStudents(String order) {
		...
		List<Student> students = new Query_GetAllStudents(dataSource).execute();
		java.util.Collections.sort(students,new StudentComparator(order));
	...
	public boolean deleteStudent(long id) {
		...
		if (new Query_DeleteStudent(dataSource).update(param) == 1) {
