...
public class JdbcStudentDAO implements IStudentDAO {
	...
	private String sql_selectStudentByEmail = "";
	 ...
	 *            SQL Statement as String
	 */
	public void setSql_selectStudentByEmail(String sql_selectStudentByEmail) {
		this.sql_selectStudentByEmail = sql_selectStudentByEmail;
	}
	...
	public Student saveStudent(final Student student) {
		log.info("Add Student Name = " + student.getFullname());
		log.info("Student username = " + student.getEmail());
		
		if (!userDAO.userExists(student.getEmail()))
				return null;
		...
		simpleJdbcTemplate.update(sql_insertStudent, student.getMatnr(),
				...
				student.getEmail());
		/*
		 * activate the following error line to create an Error which terminates
		 * this method. One will see, that the complete transaction is rolled
		 * back, hence the insert statement above is not executed, alternatively
		 * the second rollback statement can be activated with the same result
		 * which in that case is a manual rollback of the transaction
		 */
		...
		/*
		 * result from query is a list, actually containing only one row and one
		 * column
		 */
	...
	public boolean deleteStudent(int id) {
		...
		log.error("Deletion Student ID = " + id + " failed");
		return false;
	}
	...
	public Student getStudentByEmail(String email) {
		...
		List<Student> students = simpleJdbcTemplate.query(sql_selectStudentByEmail, new StudentMapper(), email);
		if (students.size() == 1) {
			Student s = students.get(0);
			log.debug("Returning Student \"" + s.getFirstname() + " "
					+ s.getLastname() + "\"");
